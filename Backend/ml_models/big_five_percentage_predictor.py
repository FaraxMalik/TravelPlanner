...
class BigFivePercentagePredictor:
    ...
    def load_model(self, filepath="big_five_percentage_model.pkl"):
        ...
        # print(f"Model loaded from {filepath}")  # Commented out to avoid interfering with JSON output

def main():
    """Main function with command line argument support"""
    parser = argparse.ArgumentParser(description='Big Five Personality Predictor')
    ...
    parser.add_argument('--responses', type=str, help='JSON string of 12 responses (0-3)')
    parser.add_argument('--model', type=str, default='big_five_percentage_model.pkl', help='Model file path')
    ...
    if args.train:
        ...
        if not args.responses:
            print("Error: --responses required for prediction")
            sys.exit(1)
        ...
        try:
            ...
            if len(responses) != 12:
                print("Error: Expected 12 responses, got", len(responses))
                sys.exit(1)
            
            # Make prediction
            result = predictor.predict_big_five_percentages(responses)
        ...
        print("Error: Please specify --train or --predict")
        sys.exit(1)
