#!/usr/bin/env python3
"""
🚀 Enhanced Backend Integration Function with Weather Data
...
def generate_itinerary_for_backend(travel_preferences, destination, travel_dates, duration, daily_budget, additional_preferences=""):
    """
    Enhanced function for Node.js backend integration with weather data
    ...
    Returns:
        JSON response with itinerary including weather data
    """
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # Test mode with weather integration
        print("🎯 ENHANCED BACKEND INTEGRATION TEST (WITH WEATHER)")
        print("=" * 60)
        ...
        result = generate_itinerary_for_backend(
            ...
            5, 
            120,
            "Want to see Eiffel Tower and Louvre, prefer French cuisine"
        )
        
        if result['success']:
            print("✅ SUCCESS! Enhanced itinerary with weather data generated")
            ...
            if result['weather_forecast']:
                weather_summary = str(result['weather_forecast'].get('summary', 'Weather data available'))
                print(f"🌤️  Weather: {weather_summary[:100]}...")
            
            print(f"📋 Itinerary preview: {result['detailed_itinerary'][:200]}...")
        else:
            print(f"❌ Error: {result['error']}")
        
        print(f"\n💡 Enhanced integration ready for Node.js backend with weather data!")
